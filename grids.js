    ...
    $().w2grid({
        ...
        columns: [
            { field: 'aeronave', caption: 'Aeronave', size: '10%', sortable: true,
              resizable: true, editable: {type: 'list', items: listaAeronaves} },
    ...
    $().w2grid({
        ...
        columns: [
            ...
            { field: 'destino', caption: 'Destino', size: '20%', resizable: true,
              editable: {type: 'list', items: listaDepComb} },
            { field: 'contadorinicial', caption: 'Contador Inicial', size: '15%', resizable: true,
              editable: {type: 'number'} },
            { field: 'contadorfinal', caption: 'Contador Final', size: '15%', resizable: true,
              editable: {type: 'number'} },
    ...
    $().w2grid({
        ...
        columns: [
            ...
            { field: 'vigente', caption: 'Vigente desde', size: '25%', resizable: true,
              editable: {type: 'date'} },
            { field: 'importe', caption: 'Importe', size: '25%', resizable: true,
              editable: {type: 'number'}, render: 'float:2'},
            { field: 'descuento', caption: 'descuento(%)', size: '25%', resizable: true,
              editable: {type: 'number'}, render: 'float:2'}
        ],
    ...
    $().w2grid({
        ...
        columns: [
            { field: 'socio', caption: 'Socio/Cuenta', size: '25%', sortable: true,
              resizable: true, editable: {type: 'list', items: listaCuentas } },
            { field: 'fecha', caption: 'Fecha', size: '15%', resizable: true,
              editable: {type: 'date'} },
...
function gridSaldos() {
    ...
    for (let socio of listaSocios) {
        let saldo = 0.0;
        for (let mov of data['movs'].records) {
            if (mov.socio == socio) {
                saldo += mov.haber - mov.debe;
            }
        }
        saldos.records.push({recid: rec.id, socio: rec.text, saldo: saldo});
    }
