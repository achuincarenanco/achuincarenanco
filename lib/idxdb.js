...
class idxDB {
    ...
    openDB(version) {
        ...
        request.onupgradeneeded = (event) => {
            ...
            var storeNames = Array.from(this.db.objectStoreNames);
            console.log('store names:' + storeNames);
            for (var s of this.stores) {
                if ( storeNames.indexOf(s) == -1 ) {
    ...
    // get all records from store, then call callback function
    getRecords(store, cb) {
        var request = this.getStore(store).getAll();
        request.onsuccess = () => {
            cb(request.result);
        }
    }
